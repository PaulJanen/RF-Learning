...
class WalkerManager():
    ...
    def __init__(self):     
        ...
        self.trainAfterSteps = self.walker.env.max_episode_steps*2
        self.trainingCycles = self.walker.env.max_episode_steps
    ...
    def CreateAndStartWalkers(self):
        if(len(self.allWalkers) > 0):
            ...
            for index,i in enumerate(range(5555,5565)):
                self.allWalkers.append(Walker(i, self, oldWalkers[index]))    
        else:
